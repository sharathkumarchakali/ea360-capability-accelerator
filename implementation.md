...
A world-class EA product needs module-specific interaction models.
...
Do not overload the table with full descriptions.
...
This is essential to differentiate EA360 from assessment-only products.
...
> API adoption is growing, but lifecycle governance and ownership remain inconsistent.
...
Show supporting evidence and missing evidence.
...
Make EA360 feel evidence-backed rather than questionnaire-driven.
...
A user must be able to open evidence and see exactly what it supports.
...
Cards should no longer be isolated content blocks.
...
## 13.1 Architecture Reviews

Add a queue experience:
...
Highlight expiring high-risk exceptions.
...
A technology radar-style visual can be P1.
...
For P0, clarity matters more than free-form graph editing.
...
This proves the experience without pretending AI is implemented.
...
Every record should have stable IDs.

Do not use record names as identifiers.
...
For demo safety, prefer fictional organisations as the default seed packs.
...
This should establish the product behaviour even before a real database exists.
...
World-class table behaviour for important registers:
...
Do not implement table complexity on every small view.

## Drawers

Use right-side drawers for fast record inspection.

Drawers should preserve user context and support linked navigation.

## Create / Add

Replace placeholder toast with contextual modal.

For prototype:

- create record in memory/localStorage
...
- table CSV
- data JSON where useful
- Executive Report print stylesheet / browser PDF

Do not promise enterprise reporting features that do not exist.
...
Avoid jargon-first copy.

TOGAF can inform the model but should remain behind the interface.
...
Do not refactor everything before delivering user value.

Refactor incrementally while implementing each vertical slice.
